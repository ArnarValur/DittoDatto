...
///
/// Actual image bytes live in an object store (Firebase Storage for PoC,
/// European S3-compatible in future). The [url] field contains the
/// download URL. The storage backend is swappable via [MediaStorageBackend].
class MediaItem {
  const MediaItem({
    required this.id,
  ...
  /// Object store download URL.
  final String url;

  /// Full storage path for deletion (e.g. `companies/slug/media/1234-photo.jpg`).
  ///
  /// **Path isolation rule:** Must always be under `companies/{slug}/media/`.
  /// Never write outside the `/media/` subfolder — the same bucket is shared
  /// with the live Nuxt media manager.
