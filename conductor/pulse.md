...
**Session Focus:** BP auth email validation fix + RECORD ACCESS permissions fix. Proper test-first deployment.

## 🚀 Active Tracks

- **BP Login + Establishments** (`bp_login_establishments_20260614`) — In-progress. Phases 1–4 complete. Auth now fully working E2E on Saturn: email+password login, full email validation (no prefix-only matching), correct company DB scoping. **Deployed and user-verified.**
- **Admin Panel** (`admin_panel_20260527`) — In-progress. All 5 phases complete in plan. Auth is NS-level (VPN-only, ADR-0007). Needs rebuild+redeploy to pick up DB consolidation + password field changes.

## ✅ Recently Completed

- **2026-06-19** — BP auth email validation fix: Changed `bp_auth` SIGNIN from username-prefix matching to full email matching. Fixed SCHEMAFULL table permissions for RECORD ACCESS tokens (`PERMISSIONS FOR select WHERE id = $auth.id`). 46 tests green (21 integration + 25 unit). Deployed and verified on Saturn.
- **2026-06-19** — BP Auth full-stack fix: DB consolidation `profiles` → `users`, argon2 password hashing, Admin Panel password fields, Saturn DB migration (3 users + bp_auth ACCESS + bp_portal service users), BP web build.
