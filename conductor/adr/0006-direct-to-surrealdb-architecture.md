...
2. **MercuryEngine (Booking Engine):** Operates purely on the booking hold/booking lifecycle and Time Tetris calculator. It does not issue tokens, handle OIDC, or expose admin endpoints. It accesses SurrealDB using its own high-privilege service account credentials (`mercury@companies`, `mercury@users`), executing booking transactions with **Delegated Trust**.
