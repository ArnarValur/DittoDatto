...
| **StaffMember** | A person who performs Services at an Establishment. | (none) |
...
| **BankID** | Norwegian digital identity via Vipps Login (OIDC). Mandatory for consumer transactions, not for walk-in accounts. | ID verification (avoid) |
...
| **SolarTheme** | Time-of-day theme switching based on solar position. Future `ditto_design` layer, not v1. | night mode |
...
| Public Marketplace | webapp, frontend |
...
| `companies` / `users` namespace | `titan` / `enceladus` |
...
### `/grill mercury-engine`
- Pydantic v2 datetime conventions
- Schema migration

### Open domain refinements
- `Favorite.type` — staff favoriting scope
- MasterDatto — scope and entity model
- SearchEvent schema extensions
