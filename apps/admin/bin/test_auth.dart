import 'dart:io';
import 'package:surrealdb/surrealdb.dart';

/// Command-line verification tool for DittoDatto Hub authentication.
///
/// Validates:
/// 1. WebSocket connection to Caddy proxy /rpc endpoint.
/// 2. Incorrect credentials rejection (security check).
/// 3. Correct namespace user signin & JWT token generation.
void main(List<String> args) async {
  final host = args.isNotEmpty ? args[0] : '100.87.99.59';
  final url = 'ws://$host:8002/rpc';

  print('📡 Connecting to SurrealDB at $url...');
  final db = SurrealDB(url);
  
  try {
    db.connect();
    await db.wait().timeout(const Duration(seconds: 5));
    print('✅ Connection established.');
  } catch (e) {
    print('❌ Connection failed: $e');
    exit(1);
  }

  // Test 1: Incorrect credentials must FAIL
  print('\n🔒 Test 1: Verifying that incorrect login fails...');
  try {
    await db.signin(
      user: 'arnarvalur',
      pass: 'wrongpassword',
      namespace: 'users',
    );
    print('❌ FAIL: Incorrect credentials allowed signin!');
    exit(1);
  } catch (e) {
    print('✅ SUCCESS: Incorrect login was rejected as expected: $e');
  }

  // Test 2: Correct credentials must SUCCEED
  print('\n🔑 Test 2: Verifying that correct login works...');
  try {
    final token = await db.signin(
      user: 'arnarvalur',
      pass: 'admin123',
      namespace: 'users',
    );
    if (token.isNotEmpty) {
      print('✅ SUCCESS: Correct login succeeded. Token generated.');
    } else {
      print('❌ FAIL: Correct login succeeded but token was empty.');
      exit(1);
    }
  } catch (e) {
    print('❌ FAIL: Correct login rejected: $e');
    exit(1);
  }

  db.close();
  print('\n🎉 All authentication validation tests passed successfully!');
  exit(0);
}
