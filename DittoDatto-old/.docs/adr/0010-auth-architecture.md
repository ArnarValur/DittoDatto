---
title: "ADR-0010: Auth Architecture"
type: "adr"
status: "accepted"
date: "2026-05-05"
session: 14
...
**MercuryEngine owns the auth pipeline end-to-end.** SurrealDB is the user *store* (`enceladus/users`), not the auth issuer.
...
**Registered power users** (v1.5+ — agentic second-sale stores) will require BankID when that tier is introduced.
...
User record in `enceladus/users` stores: `vipps_sub`, `name`, `email`, `phone`.
...
SurrealDB can independently validate agent JWTs via:
```surql
...
- MercuryEngine calls `db.use(namespace="titan", database=f"company_{slug}")` per request
...
| **`require_operator`** | `require_auth` + `role == operator` + company access guard | Service/staff CRUD, dashboard |

**Company access guard:** Operators → `token.company_slug == path.slug`. Agents → `slug in token.allowed_companies`.

Request flow:
```
Authenticated: Request → require_auth → company_access → handler → db.use(company_{slug})
Guest:         Request → (public) → handler → db.use(company_{slug}) → inline guest info
```

### 8. Vipps OIDC for Production, Dev Bypass for Development

**Production:** All operator auth goes through Vipps Login (browser-based OIDC with PKCE).

**Development/Staging:** A `POST /auth/dev-login` endpoint (email + password) is registered only when `environment == "development"`. Returns the exact same JWT format as Vipps would. Gated by environment flag — physically absent in production builds.

Dev user is seeded during database bootstrap.

## Architecture

```
Flutter App / Business Portal
    │
    ├─ Operator: Vipps OIDC (PKCE) ──→ MercuryEngine /auth/vipps
    │       exchange code → fetch userinfo → upsert enceladus/users → mint JWT
    │
...
MercuryEngine (FastAPI)
    ...
    └── DB: db.use("titan", f"company_{slug}")

SurrealDB 3.0
    ├── DEFINE ACCESS TYPE JWT (defense-in-depth validation)
    ├── titan/company_{slug} — business data
    └── enceladus/users — operator profiles (vipps_sub, name, email)
```
...
- Dev bypass enables fast iteration without Vipps sandbox
...
*Origin: Session 14 Auth Grill — 8 decisions locked (2026-05-05)*
