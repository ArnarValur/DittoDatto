...
// Use Firestore profile for name + avatar (not Firebase Auth fallback)
...
const user = computed(() => {
  ...
  return {
    name: p?.name || u?.displayName || u?.email?.split('@')[0] || 'User',
    avatar: {
      src: p?.photoUrl || u?.photoURL || '',
      alt: p?.name || u?.displayName || 'User'
    }
  }
})
