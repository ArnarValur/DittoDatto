...
| `company-blueprint.surql` | `titan/company_{slug}` | Template applied per company onboard |
| `discovery.surql` | `titan/discovery` | DittoBar search, categories, areas, demand signals |
| `users.surql` | `enceladus/users` | Consumer profiles, favorites, booking refs |
| `platform.surql` | `titan/platform` | Company registry, system alerts, audit log |
...
surreal sql --endpoint http://localhost:8000 -u root -p <pw> --ns titan --db discovery < schemas/discovery.surql
surreal sql --endpoint http://localhost:8000 -u root -p <pw> --ns titan --db platform < schemas/platform.surql
surreal sql --endpoint http://localhost:8000 -u root -p <pw> --ns enceladus --db users < schemas/users.surql

# 3. Provision a company (example: Sawasdee)
surreal sql --endpoint http://localhost:8000 -u root -p <pw> --ns titan -c "DEFINE DATABASE IF NOT EXISTS company_sawasdee"
surreal sql --endpoint http://localhost:8000 -u root -p <pw> --ns titan --db company_sawasdee < schemas/company-blueprint.surql
```
